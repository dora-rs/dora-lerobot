...
def build_physical_to_logical(table: {str: int}):
    ...
    two_seventy_to_zero = select_limits_physical_to_logical(table["270"], table["0"])
...
def select_limits_logical_to_physical(a, b):
    if abs(b - a) > 90:
        if a > 0 > b:
            return a, b % 360
        if a < 0 < b:
...
def build_logical_to_physical(table: {str: int}):
    minus_one_eighty_to_minus_ninety = select_limits_logical_to_physical(table["-180"], table["-90"])

    minus_ninety_to_zero = select_limits_logical_to_physical(table["-90"], table["0"])

    zero_to_ninety = select_limits_logical_to_physical(table["0"], table["90"])

    ninety_to_one_eighty = select_limits_logical_to_physical(table["90"], table["-180"])
...
def build_physical_to_logical_tables(physical_position_1, physical_position_2, wanted) -> [{str: int}]:
    ...
    for i in range(len(physical_position_1)):
        ...
        wanted_first, wanted_second = wanted_first * 360 / 4096, wanted_second * 360 / 4096
