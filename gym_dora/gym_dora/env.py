...
class DoraEnv(gym.Env):
    ...
    def __init__(self, fps: int, actions: list[str], joints: list[str] | None = None, cameras: dict[str, tuple[int]] | None = None):
        self.fps = fps
        ...
        if self.cameras:
            ...
            observation_space["pixels"] = pixels_space

        self.observation_space = spaces.Dict(observation_space)
        self.action_space = spaces.Box(low=-1, high=1, shape=(len(self.actions),), dtype=np.float32)
        ...
        while True:
            ...
            if event["type"] == "INPUT":
                ...
                if "cam" in event["id"]:
                    ...
                    self._observation["pixels"][camera] = (
